    ...
    protected function getHeaderActions(): array
    {
        return [
            Actions\EditAction::make()
                ->color('info')
                ->visible(fn ($record) => auth()->user()->hasAnyRole(['Admin', 'Super Admin']) ||
            ...
            Actions\Action::make('send_renewal_view')
                ...
                ->color('warning')
                ->visible(fn ($record) => auth()->user()->hasAnyRole(['Super Admin', 'Admin']) &&
                    $record->next_calibration_date &&
                    Carbon::parse($record->next_calibration_date)->isFuture()
                )
                ->action(function ($record) {
                    ...
                    if ($admins->isNotEmpty()) {
                        \Illuminate\Support\Facades\Notification::send($admins, new CalibrationRenewalNotification(collect([$record])));
